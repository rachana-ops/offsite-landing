...
function htmlFiles(directory) {
  return readdirSync(directory).flatMap((name) => {
    // Advertorials route between pretranslated HTML files. /unlock and
    // /lem-lander are standalone English-only builds, not shared JSON pages.
    if (["advertorial", "unlock", "lem-lander"].includes(name) || name.startsWith(".")) return []
...
test("every shared bridge page has complete dictionaries for every locale", () => {
  ...
  for (const file of htmlFiles(rootPath)) {
    // The homepage is the English-only Lem lander, so it carries no shared
    // dictionary even though localized legacy bridge pages still do.
